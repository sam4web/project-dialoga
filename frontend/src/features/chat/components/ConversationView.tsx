...
function ConversationView({ userId }: { userId: string }) {
  ...
  const { executeAction } = useActionWithToast<IUser, string>();
  ...
  useEffect(() => {
    const fetchPublicUserProfile = async () => {
      ...
      setSelectedProfile(userProfile as IUser);
    };
  ...
  useTitle({ title: `${selectedProfile ? `${selectedProfile.fullname}'s` : ""} Chat`, template: true });
